const VideoSection = ({dict}) => {
  return (
    <section className="relative mt-10 flex flex-col items-center justify-center bg-gradient-to-br from-white via-[#f5f7fa] to-white py-24 px-4 overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col items-center w-full max-w-3xl bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl border border-gray-200 p-8 md:p-12">
        <h2 className="text-gray-900 text-4xl md:text-5xl font-extrabold mb-4 tracking-wide drop-shadow-lg text-center">
          {dict.video.title}
        </h2>
        <p className="text-gray-700 text-lg md:text-xl mb-8 text-center max-w-2xl drop-shadow">
          {dict.video.description}
        </p>
        <div className="w-full aspect-video rounded-xl overflow-hidden shadow-lg border-2 border-gray-200 bg-black">
          <iframe
